...
// localConsole starts a new em node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote em instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running em instance and start the JavaScript console
	...
	if endpoint == "" {
		...
		endpoint = fmt.Sprintf("%s/em.ipc", path)
	}
...
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		...
		// Backwards compatibility with em < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new em node, attaches an ephemeral JavaScript
