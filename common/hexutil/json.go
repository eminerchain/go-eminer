...
func (b Bytes) MarshalEmText() ([]byte, error) {
	result := make([]byte, len(b)*2+2)
...
func (b *Bytes) UnmarshalText(input []byte) error {
	raw, err := checkDacText(input, true)
	if err != nil {
		return err
	}
...
func UnmarshalFixedUnprefixedText(typname string, input, out []byte) error {
	raw, err := checkDacText(input, false)
	if err != nil {
		return err
	}
	if len(raw)/2 != len(out) {
...
func bytesHaveEMPrefix(input []byte) bool {
...
func checkDacText(input []byte, wantPrefix bool) ([]byte, error) {
	if len(input) == 0 {
		return nil, nil // empty strings are allowed
	}
	if bytesHaveEMPrefix(input) {
		input = input[2:]
...
func checkNumberText(input []byte) (raw []byte, err error) {
	...
	if !bytesHave0xPrefix(input) && !bytesHaveEMPrefix(input) {
		return nil, ErrMissingPrefix
	}
	if bytesHave0xPrefix(input) {
		input = input[2:]
	} else if bytesHaveEMPrefix(input) {
		input = input[2:]
	}
