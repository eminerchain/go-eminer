...
func FromEmHex(s string) []byte {
	if len(s) > 1 {
		if strings.EqualFold(s[0:2], AddressPrefixLower) {
			s = s[2:]
		}
	}
	return Hex2Bytes(s)
...
func hasEMPrefix(str string) bool {
	return len(str) >= 2 && (str[0] == 'e' || str[0] == 'E') && (str[1] == 'M' || str[1] == 'm')
}
