...
// Package emstats implements the network stats reporting service.
package emstats
...
// Service implements an eminer-pro netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server      // Peer-to-peer server to retrieve networking infos
	dac    *em.Dacchain     // Full eminer-pro service if monitoring a full node
...
func New(url string, dacServ *em.Dacchain) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // emstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["em"]; info != nil {
		network = fmt.Sprintf("%d", info.(*em.NodeInfo).Network)
		protocol = fmt.Sprintf("em/%d", em.ProtocolVersions[0])
	} else {
	}
	auth := &authMsg{
		Id: s.node,
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the emstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to emstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to emstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to emstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to emstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to emstats")
