...
import (
	...
	aa "github.com/eminerchain/go-eminer/accounts/walletType"
	"github.com/eminerchain/go-eminer/common"
	"github.com/eminerchain/go-eminer/common/hexutil"
	"github.com/eminerchain/go-eminer/common/math"
	"github.com/eminerchain/go-eminer/common/ntp"
...
func (s *PublicBlockChainAPI) GetDetailBalance(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (map[string]string, error) {
	...
	result["EM_balance"] = balance.String()
	lockBalance := state.GetLockBalance(address)
	result["EM_lockBalance"] = lockBalance.String()
	totalBalance := balance.Add(balance, lockBalance)
	result["EM_totalBalance"] = totalBalance.String()
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	var signer types.Signer = types.NewDacchainSigner(tx.ChainId())
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	if args.To != "" {
		match, err := regexp.MatchString("(?i:^EM|0x)[0-9a-fA-F]{40}([0-9A-Za-z]{0}|[0-9A-Za-z]{32})$", args.To)
		if err != nil {
			return err
		}
		if !match {
		...
		if len(args.To) > strictAddresLength {
			...
			match, err := regexp.MatchString("(?i:^EM|0x)[0-9a-fA-F]{40}([0-9A-Za-z]{0}|[0-9A-Za-z]{32})$", subAddress)
			if err != nil {
				return err
			}
			if !match {
...
func (args *SendTxArgs) toTransaction() (*types.Transaction, error) {
	...
	default:
		if !common.IsHexAddress(args.To) && !common.IsEMAddress(args.To) {
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (common.Hash, error) {
	...
	for _, p := range pending {
		var signer types.Signer = types.NewDacchainSigner(p.ChainId())
