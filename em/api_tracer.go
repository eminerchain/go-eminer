...
import (
	...
	"github.com/eminerchain/go-eminer/em/tracers"
...
type ephemeralDatabase struct {
	diskdb emdb.Database     // Persistent disk database to fall back to with reads
	memdb  *emdb.MemDatabase // Ephemeral memory database for primary reads and writes
}
...
func (db *ephemeralDatabase) NewBatch() emdb.Batch {
...
func (db *ephemeralDatabase) Prune(root common.Hash) {
	...
	db.memdb, _ = emdb.NewMemDatabaseWithCap(db.memdb.Len())
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &emapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  emapi.FormatLogs(tracer.StructLogs()),
		}, nil
