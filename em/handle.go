...
import (
	...
	"github.com/eminerchain/go-eminer/em/fetcher"
...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb emdb.Database, taskManager *DposTaskManager, blockChan chan *types.Block, addDelegateWalletCallback func(data *aa.DelegateWalletInfo), delegateWallets map[string]*ecdsa.PrivateKey) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < em02 {
			continue
		}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= em02 && msg.Code == GetNodeDataMsg:
		return pm.dealGetNodeDataMsg(msg, p)
	case p.version >= em02 && msg.Code == NodeDataMsg:
		return pm.dealNodeDataMsg(msg, p)
	case p.version >= em02 && msg.Code == GetReceiptsMsg:
		return pm.dealGetReceiptsMsg(msg, p)
	case p.version >= em02 && msg.Code == ReceiptsMsg:
