...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (emdb.Database, error) {
	if ctx.config.DataDir == "" {
		return emdb.NewMemDatabase()
	}
	db, err := emdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
