...
func WriteCanonicalHash(db emdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db emdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db emdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db emdb.Putter, hash common.Hash) error {
...
func WriteHeader(db emdb.Putter, header *types.Header) error {
...
func WriteBody(db emdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db emdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db emdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db emdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db emdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db emdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db emdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func WriteDelegateBodyRLP(db emdb.Putter, rlp rlp.RawValue) error {
...
func WriteDelegateShuffleBlockHeightRLP(db emdb.Putter, rlp rlp.RawValue) error {
...
func PreimageTable(db emdb.Database) emdb.Database {
	return emdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db emdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db emdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db emdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
